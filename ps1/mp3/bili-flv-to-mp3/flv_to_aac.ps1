# flv转mp3,音频 aac 转 mp3,单声道，vbr 128k,
...
function flvtom4a {
    ...
    foreach ($file in $files)
    {
        ...
        # 输出转换后的文件名
        $outputFilePath = Join-Path -Path $file_path -ChildPath "$($file.BaseName).mp3"

        # 使用ffmpeg进行转换
        ffmpeg -i $file.FullName -vn  -acodec mp3 -ac 1 -q:a 2 -b:a 128k   $outputFilePath
